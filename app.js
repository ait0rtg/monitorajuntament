...
const UIRenderer = (() => {
  ...
  function renderDetailModal(record) {
    ...
    const sections = [
      ...
      record.observacions ? `
        <div class="modal__detail-row">
          <div class="modal__detail-label">Observacions</div>
          <div class="modal__detail-value">${record.observacions}</div>
        </div>` : '',
      divider,
      `<div class="modal__detail-row">
        ...
        <div class="modal__detail-value">
          <a href="${record.url_original || '#'}" target="_blank" rel="noopener noreferrer"
             class="modal__detail-value--mono" style="color:var(--color-accent-blue);">
...
const ModalController = (() => {
  ...
  async function saveDetail() {
    ...
    try {
      await DataService.updateRecord(_currentRecordId, { estat_seguiment: estatSelect.value });
      ToastService.show('Estat actualitzat correctament.', 'success');
...
const Router = (() => {
  ...
  const VIEW_IDS = ['dashboard', 'novetats', 'seguiment', 'recordatoris', 'revisions', 'imports', 'preguntes', 'fonts'];
  ...
  function _renderCurrentView(records) {
    ...
    switch (_currentView) {
      case 'dashboard':
        ...
        UIRenderer.renderLiveTable(sorted);
        UIRenderer.renderUrgencyChart(kpis);
        UIRenderer.renderCalendar(records);
      ...
      case 'seguiment':
        ...
        break;
      case 'recordatoris':
        UIRenderer.renderRecordatoris(records);
        break;
      case 'revisions':
        UIRenderer.renderRevisions(records);
        break;
      case 'imports':
        UIRenderer.renderImports(records);
        break;
      case 'preguntes':
        UIRenderer.renderPreguntes(records, _currentPregFilter);
        break;
      case 'fonts':
